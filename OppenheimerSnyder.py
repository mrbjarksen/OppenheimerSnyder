...
class OppenheimerSnyder(Scene):
    def construct(self):
        ...
        # a = R_0/2 * PI/2 * (1 + Delta_tau**2) * f0 / (R_0 - 2*M)
        a = 1/3
        f = lambda t: 1 - 2/PI * np.arctan(a*(t - Delta_tau))
        g = lambda t: f(t) * (R_0 - 2*M) / f(0) + 2*M

        print(g(0), g(Delta_tau), g(1e12))

        star2.add_updater(lambda mob: mob.scale_to_fit_height(
            # star_scale * 2 * (R_0 - (R_0 - 2*M) * 2/PI * np.arctan(tau.get_value()-10)
            # star_scale * (1 - np.arctan(0.5*tau.get_value() - 10))
